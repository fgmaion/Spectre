int FFTlog_memory(int FFTlogRes, double loval, double hival, double beta, double velDispersion){
    ...
    FFTLog_setInput(  oct_config,      beta, velDispersion);
  ...
  for(i=0; i<fc->N; i++){
    ...
    fc->pk[i][0]        = 
                          ...
                          *pow(   bsigma8,  2.) 
                          ...
                          // *toyRSD_unitLegendreCoefficient(transformOrder);
...
int clipquad(FFTLog_config* clip, FFTLog_config* mono, FFTLog_config* quad, FFTLog_config* hex, FFTLog_config* oct, double u0, double var){
    ...
    for(i=0; i<clip->N; i++){
      ...
      clip->xi[i][0]  += (C_n(u0, 1)/var)*(2.*mono->xi[i][0]*quad->xi[i][0] + (2./7.)*pow(quad->xi[i][0], 2.) + (4./7.)*quad->xi[i][0]*hex->xi[i][0] + (100./693.)*pow(hex->xi[i][0], 2.) + (50./143.)*hex->xi[i][0]*oct->xi[i][0] 
					                                    + (14./143.)*pow(oct->xi[i][0], 2.));
    }
...
int cnvldmonoCorr(FFTLog_config* cnvld, FFTLog_config* mono, FFTLog_config* quad, FFTLog_config* hex, FFTLog_config* oct, FFTLog_config* dec, FFTLog_config* dodeca){
 ...
 for(i=0; i<mono->N; i++){
  cnvld->xi[i][0]   =         mono->xi[i][0]*(*pt2maskMultipoles)(mono->krvals[i][1], 0);     
      
  cnvld->xi[i][0]  +=  (1./5.)*quad->xi[i][0]*(*pt2maskMultipoles)(mono->krvals[i][1], 2);       
    
  cnvld->xi[i][0]  +=  (1./9.)*hex->xi[i][0]*(*pt2maskMultipoles)(mono->krvals[i][1], 4);
...
int cnvldquadCorr(FFTLog_config* cnvld, FFTLog_config* mono, FFTLog_config* quad, FFTLog_config* hex, FFTLog_config* oct, FFTLog_config* dec, FFTLog_config* dodeca){
 ...
 for(i=0; i<mono->N; i++){      
  cnvld->xi[i][0]  = mono->xi[i][0]*(*pt2maskMultipoles)(mono->krvals[i][1], 2);
      
  cnvld->xi[i][0] += quad->xi[i][0]*((*pt2maskMultipoles)(mono->krvals[i][1], 0)+(2./7.)*(*pt2maskMultipoles)(mono->krvals[i][1], 2)+(2./7.)*(*pt2maskMultipoles)(mono->krvals[i][1], 4));  
      
  cnvld->xi[i][0] += hex->xi[i][0]*((2./7.)*(*pt2maskMultipoles)(mono->krvals[i][1], 2)+(100./693.)*(*pt2maskMultipoles)(mono->krvals[i][1], 4)+(25./143.)*(*pt2maskMultipoles)(mono->krvals[i][1], 6));
