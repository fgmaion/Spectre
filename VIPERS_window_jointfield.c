...
int prep_VIPERS_jmaskMultipoles(){
    ...
    VIPERS_jmask_lineNo_hihi = 0;
    ch                       = 0;

    do{
        ch = fgetc(inputfile);

        if(ch == '\n')  VIPERS_jmask_lineNo_hihi += 1;
    ...
    VIPERS_jmask_lineNo_hi = 0;
    ch                     = 0;

    do{
        ch = fgetc(inputfile);

        if(ch == '\n')  VIPERS_jmask_lineNo_hi += 1;
    ...
    VIPERS_jmask_lineNo_lo = 0;
    ch                     = 0;

    do{
        ch = fgetc(inputfile);
        if(ch == '\n')  VIPERS_jmask_lineNo_lo += 1;
    } while(ch != EOF);
