int CalcCorrections(){  
    ...
    for(j=0; j<n0*n1*n2; j++) fkpWeightedVolume  += FKPweights[j]*(TotalVolume/TotalSurveyedVolume)*Cell_AppliedWindowFn[j]*CellVolume;
    for(j=0; j<n0*n1*n2; j++) fkpSqWeightsVolume += pow(FKPweights[j]*(TotalVolume/TotalSurveyedVolume)*Cell_AppliedWindowFn[j], 2.)*CellVolume;
