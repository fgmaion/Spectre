int CalcCorrections(){  
    ...
    for(j=0; j<n0*n1*n2; j++) fkpWeightedVolume  += FKPweights[j]*(TotalVolume/TotalSurveyedVolume)*booldensity[j]*CellVolume;
    for(j=0; j<n0*n1*n2; j++) fkpSqWeightsVolume += pow(FKPweights[j]*(TotalVolume/TotalSurveyedVolume)*booldensity[j], 2.)*CellVolume;
