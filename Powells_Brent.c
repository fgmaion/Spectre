...
/*  Here ITMAX is the maximum allowed number of iterations
         ...
         ZEPS  is a small number that protects against trying to acheive fractionla accuracy for a minimum that
...
float brent(float ax, float bx, float cx, float (*f)(float), float tol, float *xmin){
    /*  Given a function f, and given a bracketing triplet of abscissas ax, bx, cx (such that bx is between ax and cx, and 
        ...
        using Brent's method.  The abscissa of the minimum is returned as xmin, and the minimumm function value is returned
        as brent, the returned function value. 
    */
    
    int iter;
    
    float a, b, d, etemp, fu, fv, fw, fx, p, q, r, toll, tol1, tol2, u, v, w, x, xm;
    
    float e = 0.0;                 // This will be the distance moved on the step before last.  a and b must be in 
                                   // ascending order, but input abscissas need not be.
    ...
    for(iter=1; iter<= ITMAX; iter++){  // Main program loop.
        ...
        if(fabs(x-xm) <= (tol2 - 0.5*(b-a))){     // Test be done here.
            *xmin = x;
            return fx;
        } 
        ...
        if(fabs(e) > tol1){             // Construct a trial parabolic fit. 
            ...
            if(fabs(p) >= fabs(0.5*q*etemp) || p <= q*(a-x) || p >= q*(b-x)){
                // Use of the ternary operator, a one line if-else statement. if a>b ? [return] a : [else return] b.
        ...
        if(fu <= fx){
            if(u >= x) a = x; else b=x;         // Now decide what to do with the fn. evaluation. 
    ...
    nrerror("Iteration limit in Brent exceeded");
