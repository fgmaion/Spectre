...
#PBS -l mem=5MB
#PBS -e /home/mjw/HOD_MockRun/chi2_log/chi2_stderr.pbs                                                                       
#PBS -o /home/mjw/HOD_MockRun/chi2_log/chi2_stdout.pbs                                                                                                     

: <<'END'
multi-line comment in bash.
END
...
set_lock(){
  ...
  while [ $locked = 1 ]
    do
      ...
      #DIR="/home/mjw/IO_lock_W"$FIELDFLAG"_"$LOZ"_"$HIZ 
      DIR=/home/mjw/IO_lock
      
      if mkdir $DIR; then
        # echo "Locking succeeded"  > $DIR/"lock_W"$FIELDFLAG"_"$LOZ"_"$HIZ".dat"
...
for k in $(seq 0.2 0.2 0.8)
  do
    ...
    export FILE="/home/mjw/HOD_MockRun/chi2_log/chi2_d0_"$d0"_W"$FIELDFLAG"_"$LOZ"_"$HIZ"_kmax_"$k".log"
      
    ./chi2.o $d0 $FIELDFLAG $LOZ $HIZ $k > $FILE 2>&1

    if [[ $(tr -d "\r\n" < $FILE | wc -c) -eq 0 ]]; then 
      printf "\n%s" "$FILE" >> /home/mjw/HOD_MockRun/chi2_log/chi2_stderr.pbs
    else
        date >> $FILE
    fi
done

#rm -r /home/mjw/HOD_MockRun/W1_Spectro_V7_4/models
#mkdir /home/mjw/HOD_MockRun/W1_Spectro_V7_4/models
