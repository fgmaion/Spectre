int set_u0(){
  if(data_mock_flag == 0){                                                                                                                                 
    // if(appliedClippingThreshold      == 1000.)  A11Sq  =      1.00; // Where is the redshift distinction; evolution of bias.                           
    // else if(appliedClippingThreshold ==   10.)  A11Sq  =   1./1.30;                                                                                    
    ...
    else if(appliedClippingThreshold ==    4.)  A11Sq  =   1./4.50;                                                                                       
  }                                                                                                                                                        
...
int snipping_amplitudeCorrection(double clipped_pk[], int len){
  ...
  for(j=0; j<len; j++){
    ...
    else if(appliedClippingThreshold ==  6.){
      clipped_pk[j]              *=   1.85;
      clipped_pk[j + mono_order] *=   1.85;
    }

    else if(appliedClippingThreshold ==  4.){
      clipped_pk[j]              *=   3.00;
...
int snipping_shotnoise(double clipped_pk[], int len){
  for(j=0; j<len; j++){
    if(fieldFlag == 1){
      if(hi_zlim == 1.20){
	if(appliedClippingThreshold      == 1000.)  clipped_pk[j] -= 277.16;  // Correct monopole.
	else if(appliedClippingThreshold ==   10.)  clipped_pk[j] -= 243.94;  // Note: as a difference estimator, covariance does not have to explicity corrected.
	else if(appliedClippingThreshold ==    6.)  clipped_pk[j] -= 208.99;
	else if(appliedClippingThreshold ==    4.)  clipped_pk[j] -= 167.90;
      }
    }
    
    else if(hi_zlim == 1.00){
      if(appliedClippingThreshold      == 1000.)    clipped_pk[j] -= 564.00;
      else if(appliedClippingThreshold ==    4.)    clipped_pk[j] -= 227.85;
    }

    if(fieldFlag == 4){
      if(appliedClippingThreshold      == 1000.)    clipped_pk[j] -= 289.16;  // Correct monopole.
      else if(appliedClippingThreshold ==   10.)    clipped_pk[j] -= 249.26;  // Note: as a difference estimator, covariance does not have to be shot noise corrected.
      else if(appliedClippingThreshold ==    6.)    clipped_pk[j] -= 208.00;
      else if(appliedClippingThreshold ==    4.)    clipped_pk[j] -= 168.59;
    }
  }
