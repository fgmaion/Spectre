...
int main(int argc, char **argv){
  ...
  // if(data_mock_flag==0)  for(loopCount=1; loopCount<307; loopCount++)  smoothed_nbar_calc(nz_smoothRadius, 0);
  ...
  */
  ...
  //for(loopCount=38; loopCount<39; loopCount++){
    ...
    load_homogeneous_rands_window(1,  0.05, data_mock_flag); 
    ...
    // COMMENTED JAN 11, for cube work: // chi = interp_comovingDistance(zobs[j]);
    if(Jenkins_foldfactor < 2.0)  calc_clippingweights();
