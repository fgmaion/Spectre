...
int FFTLog_setInput(FFTLog_config *fc, double beta, double velDispersion){  
  ...
  for(i=0; i<fc->N; i++){
    ...
    // printf("\n%e \t %e", fc->krvals[i][0], haloModel_pk(fc->krvals[i][0], 0.7, transformOrder));
  }
...
int FFTLog_zeroInput(FFTLog_config *fc, double beta, double velDispersion){  
  ...
  for(i=0; i<fc->N; i++){
    ...
    // purely real.  Set P(k) to obtain xi(r) by inverse Hankel transform.     
    fc->pk[i][0]        = 0.0; //*toyRSD_OnePlusOneHalfMuSq(transformOrder); // (*pt2RSD_k)(fc->krvals[i][0]*velDispersion, beta, transformOrder);
    fc->pk[i][1]        = 0.0;
