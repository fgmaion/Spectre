int CovarianceMatrix(int mockNumber){
  /*
    ...
    Covariance                                          = (double **) malloc(2*(loskBinNumb-1)*sizeof(double*));
    
    for(j=0; j<2*(loskBinNumb-1); j++) Covariance[j]    = (double  *) malloc(2*(perpkBinNumb-1)*sizeof(double));

    for(j=0; j<2*(loskBinNumb-1); j++){
        for(k=0; k<2*(perpkBinNumb-1); k++){
            Covariance[j][k] = 0.0;
        }
    }
