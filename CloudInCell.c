int cic_assign(double x, double y, double z, double redshift, double p){
  // Assigns an object at (x, y, z) into a 3D density grid with weight p using Cloud-in-Cell assignment. 
  // CellSize is the cell size, and nc is the number of cells in each dimension.  
  ...
  x = (x - AxisLimsArray[0][0])/CellSize;
  y = (y - AxisLimsArray[0][1])/CellSize;
  z = (z - AxisLimsArray[0][2])/CellSize;
  ...
  meanCellRedshift[iindx+n2*iindy+n1*n2*iindz] += redshift;
  meanCellRedshift[indx +n2*iindy+n1*n2*iindz] += redshift;
  meanCellRedshift[iindx+n2*indy +n1*n2*iindz] += redshift;
  meanCellRedshift[indx +n2*indy +n1*n2*iindz] += redshift;
  meanCellRedshift[iindx+n2*iindy+n1*n2*indz ] += redshift;
  meanCellRedshift[indx +n2*iindy+n1*n2*indz ] += redshift;
  meanCellRedshift[iindx+n2*indy +n1*n2*indz ] += redshift;
  meanCellRedshift[indx +n2*indy +n1*n2*indz ] += redshift;
