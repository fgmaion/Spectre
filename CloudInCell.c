int cic_assign(int rand_ordelta, double x, double y, double z, double p){
  ...
  x = (x - AxisLimsArray[0][0])/xCellSize;
  y = (y - AxisLimsArray[0][1])/yCellSize;
  z = (z - AxisLimsArray[0][2])/zCellSize;
  ...
  if(rand_ordelta == 0){
     surveyMask[iindx+n2*iindy+n1*n2*iindz] +=hx1*hy1*hz1*p;
     surveyMask[indx +n2*iindy+n1*n2*iindz] +=hx0*hy1*hz1*p;
     surveyMask[iindx+n2*indy +n1*n2*iindz] +=hx1*hy0*hz1*p;
     surveyMask[indx +n2*indy +n1*n2*iindz] +=hx0*hy0*hz1*p;
     surveyMask[iindx+n2*iindy+n1*n2*indz ] +=hx1*hy1*hz0*p;
     surveyMask[indx +n2*iindy+n1*n2*indz ] +=hx0*hy1*hz0*p;
     surveyMask[iindx+n2*indy +n1*n2*indz ] +=hx1*hy0*hz0*p;
     surveyMask[indx +n2*indy +n1*n2*indz ] +=hx0*hy0*hz0*p;
  }

  else if(rand_ordelta == 1){
