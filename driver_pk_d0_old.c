...
int main(int argc, char **argv){
  ...
  if(fieldFlag == 1){
    ...
    // Stefano values
    stefano_trans_x         =      +100.;                                                                                                                         stefano_trans_y         =      +300.;
  ...
  kbin_no                   =        40;            
  // kbin_no                =       129; // Stefano comparison. 
  ...
  // Stefano comparison changes. 
  // linear k spacing rather than log. 
  // kbin_no changed. 
  // inverse nbar spline -> 400 from 300.
  // multipole_calc to monopole_calc in observedQuadrupole.
  ...
  // last argument: log spacing in k is 0, linear is 1.  linear will require more bins, 129 for comparing to Stefano. 
  ...
  */
  ...
  // for(loopCount=1; loopCount<26; loopCount++)  smoothed_nbar_calc(nz_smoothRadius, 1);
  
  for(loopCount=start; loopCount<start+1; loopCount++){
  //for(loopCount=38; loopCount<39; loopCount++){
    ...
    // sprintf(filepath, "%s/John/gals.dat", root_dir);

    // output = fopen(filepath, "w");

    // for(j=0; j<Vipers_Num; j++){
    //  if(Acceptanceflag[j] == true){
    //	fprintf(output, "%lf \t %lf \t %lf \t %lf \t %lf \t %lf \n", xCoor[j], yCoor[j], zCoor[j], fkp_galweight[j], clip_galweight[j], 1./sampling[j]);
    //  }
    //}
  ...
  /*
  sprintf(filepath, "%s/John/randoms.dat", root_dir);

  output = fopen(filepath, "w");

  for(j=0; j<rand_number; j++){
    if(rand_accept[j] == true){
      fprintf(output, "%lf \t %lf \t %lf \t %lf \n", rand_x[j], rand_y[j], rand_z[j], rand_weight[j]);
    }
  }                                                                                                                                                                                                                                       \
